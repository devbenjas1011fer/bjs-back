...
router.put(
  ...
  async function (req: Request, res: Response, _next: NextFunction) {
    try {
      const cust = await AppDataSource.getRepository(CLIENTE).findOne({
        where: { id: req.body.id },
      });
      cust!.nombre = req.body.nombre,
        cust!.direccion = req.body.direccion,
        cust!.numero = req.body.numero,
        cust!.correo = req.body.correo,
        cust!.id_perfil = req.user!.perfil!,
        cust!.alta = new Date();
      await AppDataSource.getRepository(CLIENTE).save(cust!);
      res.json();
    } catch (err) {
      console.log(err);
      _next(err);
...
router.post(
  ...
  async function (req: Request, res: Response, _next: NextFunction) {
    try {
      ...
      await AppDataSource.getRepository(CLIENTE).update(
